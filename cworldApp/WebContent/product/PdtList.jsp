...
<body>
<table width="350" style="border:3px solid pink;">
<tr>
<td width="*" valign="top">
	<!-- 검색조건 입력폼 -->
	...
	<div>
		<br/>
		<table id="pdtSch" cellpadding="5"><tr><th width="20%" height="50">상품<br/>분류</th>
...
<%	} 
		...
		<input type="text" name="sp" class="price" value="<%=sp %>" size="8" onkeyup="onlyNum(this);" /> ~
		<input type="text" name="ep" class="price" value="<%=ep %>" size="8" onkeyup="onlyNum(this);" />
		</td></tr>
		<tr><th height="50">상품명</th><td><input type="text" name="pi_name" id="pi_name" class="pdtCata" size="23" value="<%=name %>" style="margin-bottom:5px;" /><br/>
		<input type="button" value="상품 검색" class="pdtBtn" onclick="makeSch();" style="border:0; background:pink; width:100px; height:30px;"/>
		<input type="button" value="조건 초기화" class="pdtBtn" onclick="resetList();" style="border:0; background:pink; width:100px; height:30px;"/>
		</td></tr>
		<tr><th>특별<br/>상품</th>
		<td height="50">
		...
		<br/><input type="checkbox" name="pi_special" id="c" value="c" <%if (chkSpecial.indexOf("c") >= 0){ %> checked="checked"<%} %>><label for="c">&nbsp;HOT</label>&nbsp;&nbsp;
...
<%
   ...
   String lnk = "onclick=\"location.href='m_product_list?cpage=1";
   String lnkList = lnk + sargs + oargs + "&v=l';\" class=\"hand\"";
   String lnkGall = lnk + sargs + oargs + "&v=g';\" class=\"hand\"";
%>
<table width="350" callpadding="5" style="margin-top:10px;">
   <tr><td colspan="3" align="right" style="border:3px solid pink; padding:5px;">
      <select name="o" onchange="location.href='m_product_list?cpage=1<%=sargs + vargs%>&o=' + this.value;">
...
if (rcnt > 0 && productList.size() > 0) {  // 검색된 상품목록이 있을 경우 
   ...
   if(v.equals("g")){
		for(int i = 0 ; i < productList.size() ; i++){
			...
			%>
			<td width="<%=350/colnum %>" align="center" onmouseover="this.bgColor='#efefef';" onmouseout="this.bgColor='';">
			<a href="/cworldApp/m_product_view?piid=<%=pi.getPi_id() + args %>">
			<img src="/cworldSite/pdt_img/<%=pi.getPi_img1()%>" width="<%=350/colnum - 20 %>" height="<%=350/colnum - 20 %>" border="0"/>
			<br/><br/>
	...
	<%		
			if (( i + 1 ) % colnum > 0 && i + 1 == productList.size()){
				for( int k = 0 ; k < colnum - (productList.size() % 3) ; k++){
					out.println("<td width='" + 350/colnum + "'></td>");
				}
				out.println("</tr>");
			}
...
<%		for ( int i = 0 ; i < productList.size(); i++ ){
			...
			<td width="20%" height='100'>
				<a href="/cworldApp/m_product_view?piid=<%=productList.get(i).getPi_id() + args %>">
				<img src="/cworldSite/pdt_img/<%=productList.get(i).getPi_img1()%>" width="80" height="80" border="0"/></a>
			</td>
			<td width="30%"> <a href="/cworldApp/m_product_view?piid=<%=productList.get(i).getPi_id() + args %>"><%=productList.get(i).getPi_name() %></a>
...
</table><table width="350" style="border:3px solid pink;">
<%	}
	...
	if (cpage == 1) {
		...
		out.println("<a href='m_product_list?cpage=1" + tmpArgs + "'>[&lt;&lt;]</a>");
		out.println("&nbsp;&nbsp;&nbsp;");
		out.println("<a href='m_product_list?cpage="+ (cpage - 1) + tmpArgs + "'>[&lt;]</a>");
		out.println("&nbsp;");
	}
	for (int i = 1, j = spage; i <= bsize && j <= pcnt ; i++, j++){
		...
		if ( cpage == j ){
			...
			out.println("&nbsp;<a href='m_product_list?cpage=" + j + tmpArgs +"'>" + j + "</a>&nbsp;");		
		}
	}		
	if (cpage == pcnt) {
		out.println("&nbsp;&nbsp;[&gt;]&nbsp;&nbsp;&nbsp;[&gt;&gt;]");
		...
		out.println("<a href='m_product_list?cpage="+ (cpage + 1) + tmpArgs + "'>[&gt;]</a>");
		out.println("&nbsp;&nbsp;&nbsp;");
		out.println("<a href='m_product_list?cpage=" + pcnt + tmpArgs + "'>[&gt;&gt;]</a>");
	}
	out.println("</td></tr>");
	...
	%>
	<%
	out.println("<table width='350' callpadding='5' style=\"border:3px solid pink; margin-top:10px;\" ><tr><td align='center' valign='center' height='200'>검색된 상품이 없습니다.</td></tr>");
}
%>
