...
@WebServlet("/m_product_list")
public class PdtListCtrl extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public PdtListCtrl() { super(); }
    protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ProductListSvc pdtListSvc = new ProductListSvc();
		rcnt = pdtListSvc.getProductCount(where);
		// 검색된 상품의 총 개수로 전체 페이지 수를 구할 때 사용
		
		ArrayList<PdtInfo> pdtList = pdtListSvc.getProductList(cpage, psize, where, orderBy);
		...
		if (pcb != null && !pcb.equals("")) { //검색조건 중 대분류가 있으면
			smallList = pdtListSvc.getSmallList(pcb);
		...
		request.setAttribute("productList", pdtList);
		request.setAttribute("pageInfo", pageInfo);
		request.setAttribute("smallList", smallList);
		RequestDispatcher dispatcher =  request.getRequestDispatcher("product/PdtList.jsp");
